...
void setup() {
  ...
  xTaskCreate(OutputTask, "Outut Task", 100, NULL, 1, NULL);
}
...
void OutputTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if ((xEventGroupValue & TASK1_BIT) != 0) {
      digitalWrite(RED, digitalWrite(RED) ^ 1);
    }
    if ((xEventGroupValue & TASK2_BIT) != 0) {
      digitalWrite(YELLOW, digitalWrite(YELLOW) ^ 1);
    }
    if ((xEventGroupValue & TASK3_BIT) != 0) {
      digitalWrite(BLUE, digitalWrite(BLUE) ^ 1);
    }
  }
}
void loop() {}